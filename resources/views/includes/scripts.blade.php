...
foreach(DB::table('bookings')->where('photo', null)->where('approval_status', null)->get() as $item)
{
    ...
    if ($diff > 3600) {
        ...
        {
            $data_slot = DB::table('slot_users')->select('slot_id')->where('booking_detail_id',  $row->id)->groupBy('slot_id')->get();
            ...
            {  
                ...
                foreach ($data_slots as $row1) {
                    # code...
                    $count = DB::table('slot_users')->where('slot_id',  $item->slot_id)->where('booking_detail_id',  $row->id)->count();
            ...
            DB::table('slot_users')->where('booking_detail_id',$row->id)->delete();
        }
    }
}
@endphp
