				...
				<div class="d-flex flex-column flex-row-fluid wrapper" id="kt_wrapper">
					...
					<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
                        ...
                        <div class="d-flex flex-column-fluid">
                            ...
                            <!-- Modal -->
                            @include('app-owner.horse-breed.create')
                            @include('app-owner.horse-breed.edit')
                            <!--end::Container-->
                        </div>
                        <!--end::Entry-->
                    </div>
...
<script type="text/javascript">
    $(document).ready( function () {
        var t = $('#dataTable').DataTable({
			...
			ajax      : "{{ route('owner.horse-breed.list.json') }}",
			columns: [
                {data: 'id', name: 'id'},
                {data: 'name', name: 'name'},
		...
		$('#dataTable tbody').on( 'click', '.delete-horse', function (e) {
			...
			Swal.fire({
				...
				if (result.value) {
					$.ajax({
						url: "{{ route('owner.horse-breed.delete') }}",
						type: 'DELETE',
						dataType: 'json',
						data: {
							"id": id,
		...
		$('body').on('click', '#editData', function () {
                var id = $(this).data('id');
                $.get('{{route('owner.horse-breed')}}'+'/edit/' + id , function (data) {
                    $('#idData').val(data.id);
                    $('#name').val(data.name);
