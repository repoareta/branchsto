                    ...
                    <div class="container-fluid stable">
                        <div class="stable-body data">
                            ...
                            <div class="card mt-10">
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-data table-success" id="dataTableunapprov">
                                            <thead>
                                                <tr>
                                                <th scope="col">#</th>
                                                <th scope="col">Stable Name</th>
	...
	<script type="text/javascript">
		$(document).ready( function () {
            ...
            $('#dataTable tbody').on( 'click', '#openDetail', function (e) {
                ...
                $.ajax({
                    url: "{{ route('package_approval.detail.package') }}",
                    type: 'GET',
                    data: {
                        "id": id,
                        "_token": "{{ csrf_token() }}",
                    ...
                    success: function (response) {
                        // update stuff
                        // append value
                        $('#stable_name').val(response.name);
                        $('#package_name').val(response.stable.name);
                        $('#price').val(response.price);
                        // title
                        $('#title_modal').text('Detail Package');
                        // open modal
            ...
            $('#dataTable tbody').on( 'click', '#approv-stable', function (e) {
                ...
                var id = $(this).attr('data-id');
                $.ajax({
                    url: "{{ route('package_approval.approv.package') }}",
                    type: "POST",
                    data: {
                        id: id,
                        _token:"{{ csrf_token() }}"		
            ...
            $('#dataTable tbody').on( 'click', '#unapprov-stable', function (e) {
                ...
                var id = $(this).attr('data-id');
                $.ajax({
                    url: "{{ route('package_approval.unapprov.package') }}",
                    type: "POST",
                    data: {
                        id: id,
                        _token:"{{ csrf_token() }}"		
